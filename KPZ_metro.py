...
steps = 30000            # Number of deposition steps

# --- KPZ Parameters ---
p_side = 0.5             # Probability of lateral sticking (roughening)

# --- Metropolis Parameters ---
kT = 1.0                 # Effective temperature (controls smoothing)
J = 1.0                  # Surface tension coupling (controls smoothing)
diffusion_per_deposition = 100 # How many diffusion attempts per deposition?
...
# ----------------------------
# Local energy function (for Metropolis)
# ----------------------------
def local_energy(x, y):
    ...
    # Energy is the sum of squared height differences (surface tension)
...
def plot_surface(step):
    ...
    ax.set_title(f"2D KPZ+EW (Metropolis)\nStep = {step}")
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
...
# ----------------------------
print(f"Running 2D KPZ (p_side={p_side}) + Metropolis (kT={kT}, J={J}) Simulation...")
for step in range(steps):
    
    # --- PART 1: KPZ DEPOSITION (Roughening) ---
    x_dep, y_dep = rng.integers(0, Lx), rng.integers(0, Ly)
    
    if rng.random() < p_side:
        neighbors = [
            ((x_dep - 1) % Lx, y_dep), ((x_dep + 1) % Lx, y_dep),
            (x_dep, (y_dep - 1) % Ly), (x_dep, (y_dep + 1) % Ly)
        ]
        rng.shuffle(neighbors)
        
        for nx, ny in neighbors:
            if h[nx, ny] > h[x_dep, y_dep]:
                h[x_dep, y_dep] = h[nx, ny]  # Pre-fill the hole
                break 
                
    h[x_dep, y_dep] += 1
            
    # --- PART 2: METROPOLIS DIFFUSION (Smoothing) ---
    for _ in range(diffusion_per_deposition):
        # Pick a random site that has at least one atom
        ...
        # --- Calculate Energy Change (dE) for the hop ---
        # 1. Get energy before the hop
        ...
        if dE > 0 and rng.random() > np.exp(-dE / kT):
            # REJECT: Move is energetically unfavorable. Undo the hop.
            h[x_diff, y_diff] += 1
            h[nx, ny] -= 1
        # ELSE: (dE <= 0 or the random check passed)
            # ACCEPT: The hop is kept. The heights are already updated.
...
# (The rest of the plotting code is identical to KPZ_model_2D.py)
